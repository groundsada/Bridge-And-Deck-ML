...
def writeNoise(f,reader,df,lowNoise, highNoise,cur_stp,stp):
    ...
    for row in reader:
        low , high = min([float(x) for x in row[1:13]]),max([float(x) for x in row[1:13]])
        syn_row = [cur_stp] + [np.random.uniform(low,high) + np.random.normal(lowNoise,highNoise) for i in range(12)]
